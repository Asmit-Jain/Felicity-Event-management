...
export default function UpcomingEvents() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const res = await api.get("/registrations/upcoming");
        if (!active) return;
        setRegistrations(res.data || []);
        setMessage("");
      } catch (e) {
        if (!active) return;
        setMessage(e.response?.data?.message || "Failed to load upcoming registrations");
      }
    };
  ...
  return (
    <div className="events-container">
      <h2 className="title">Upcoming Registered Events</h2>
      {message && <p>{message}</p>}
      ...
      <div className="events-grid">
        ...
        {registrations.length === 0 && <p>No upcoming registrations.</p>}
      </div>
    </div>
  );
}
